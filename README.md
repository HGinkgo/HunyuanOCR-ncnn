...
源码仓库不内置模型权重。普通用户可以直接下载上述运行时模型包；需要复现 pnnx 转换
或适配其他 checkpoint 的开发者，请按下面的“导出 → 打包 → 构建 → 运行”流程操作。
...
完整效果展示和技术说明见
[Tencent/ncnn Discussion #6808](https://github.com/Tencent/ncnn/discussions/6808)；
本节只保留仓库内可直接核验的要求映射。
...
- 可选只读 mmap 权重加载，减少冷启动复制和匿名内存占用；默认关闭。
...
默认加载方式保持不变。当前 Linux CPU 环境的 AR load-only 五次测试中，加载时间
中位数由约 `2508 ms` 降至 `497 ms`，匿名 PSS 约减少 `1.55 GB`；三次独立进程
首次推理的 wall time 中位数由 `11.49 s` 降至 `8.77 s`，生成 token 完全一致。
...
C++ 调用方可以设置 `RuntimeOptions::mmap_weights = true`。映射页属于可回收的
file-backed 内存，但 Linux 会将已触碰的映射页计入进程 RSS/PSS，因此启用后总
RSS 可能更高；比较内存时应同时观察 `RssAnon` / `Pss_Anon` 和 file-backed 指标。
该选项减少的是加载复制和匿名内存，不会缩小模型文件，也不改变推理数值逻辑。
