...
脚本会自动发现 `./hunyuan_ocr_ncnn_model` 和相邻的 `../ncnn/build/src`，然后构建并完整识别一张双语字幕示例图：

```bash
scripts/quickstart_existing_model.sh
```

模型或 ncnn 位于其他位置时，可分别使用 `--model PATH` 和 `--ncnn-dir PATH` 覆盖。示例会持续生成到模型自然结束，不做 token 截断。
...
安装后可让模型常驻内存，连续输入图片路径而无需重复加载：

```bash
~/.local/bin/hunyuan-ocr --model ./hunyuan_ocr_ncnn_model --interactive
```

默认使用 CPU fp32；支持 Vulkan 的构建可通过 `--vulkan` 同时启用 Vision 和 Text Vulkan：
...
在仓库目录内可省略 `--model`，CLI 会自动发现 `./hunyuan_ocr_ncnn_model` 和常见 `assets/` 目录。单图默认使用 `document` prompt，识别文本会在生成过程中直接输出；默认最多生成 8192 个 token，并在 EOS 或尾部重复时提前结束。需要坐标时可显式使用 `--prompt-mode spotting`，也可以使用 `--prompt "只输出图片中的可见文字"` 传入自定义 prompt。
...
| Vulkan | `--vulkan` | Vision、Decoder 和 LM Head 使用 Vulkan；暂不兼容 DFlash。 |

Vulkan 路径需应用 [`patches/ncnn`](patches/ncnn)：

```bash
./build/hunyuan_ocr_cli --image ./document.png --vulkan
```

设备可通过 `--vulkan-device N` 选择。C++ 调用方仍可在 `RuntimeOptions` 中分别设置 Vision 和 Text Vulkan。
...
- `--vulkan` 暂不与 DFlash 组合；DFlash 仍使用 CPU fp32 文本路径。
